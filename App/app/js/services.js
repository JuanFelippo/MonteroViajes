	...
	service('$menuItems', function()
	{
		...
		var menuItemObj = {
			...
			addItem: function(title, link, icon)
			{
				...
				{
					//if(item.link.match(/^\./))
					item.state = $menuItemsRef.toStatePath(parent.state+item.link);
		...
		this.addItem = function(title, link, icon)
		{
			var item = angular.extend(angular.copy(menuItemObj), {
				title: title,
				link: link,
				state: this.toStatePathRoot(link),
				icon: icon
			});
            console.log("item link "+item.state);
			this.menuItems.push(item);
		...
		{
			//this.addItem('Login', 						'/login','linecons-cog');
			var dashboard    = this.addItem('Contratos', 		'/app', 			'linecons-cog');
			dashboard.addItem('Lista', 	'/contract/list'); // "-/" will append parents link
			dashboard.addItem('Crear', 	'/contract/create');
			var layouts      = this.addItem('Cuenta','/',	'linecons-desktop');
			layouts.addItem('Salir', 	'logout');
		...
		this.toStatePathRoot = function(path)
		{
